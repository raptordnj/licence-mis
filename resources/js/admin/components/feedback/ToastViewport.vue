    ...
    <div class="pointer-events-none fixed bottom-4 right-4 z-[60] grid w-full max-w-sm gap-2">
        <TransitionGroup name="toast">
            <article
                ...
                :key="message.id"
                class="pointer-events-auto rounded-xl border px-3 py-3 shadow-lg"
...
const toneClass = (tone: 'success' | 'error' | 'info'): string => {
    if (tone === 'success') {
        return 'border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-400/30 dark:bg-emerald-500/15 dark:text-emerald-100';
    }

    if (tone === 'error') {
        return 'border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-400/30 dark:bg-rose-500/15 dark:text-rose-100';
    }

    return 'border-cyan-200 bg-cyan-50 text-cyan-900 dark:border-cyan-400/30 dark:bg-cyan-500/15 dark:text-cyan-100';
};
</script>
